...
{
    public partial class DepressedViewModel : ObservableObject
    {
        private readonly IListChoiceService _listChoiceService;

        public DepressedViewModel(IListChoiceService listChoiceService)
        {
        ...
        private async Task NavigateToListChoice(string choice)
        {
            try
            {
                ...
                if (listChoice != null)
                {
                    ...
                    await Shell.Current.GoToAsync($"{nameof(ListChoice)}?emotion=Depressed&choice={Uri.EscapeDataString(listChoice.Title)}");
                }
                else
                {
