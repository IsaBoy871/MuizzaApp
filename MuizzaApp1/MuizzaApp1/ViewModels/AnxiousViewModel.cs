...
{
    public partial class AnxiousViewModel : ObservableObject
    {
        private readonly IListChoiceService _listChoiceService;

        public AnxiousViewModel(IListChoiceService listChoiceService)
        {
        ...
        private async Task NavigateToListChoice(string choice)
        {
            try
            {
                ...
                if (listChoice != null)
                {
                    ...
                    await Shell.Current.GoToAsync($"{nameof(ListChoice)}?emotion=Anxious&choice={Uri.EscapeDataString(listChoice.Title)}");
                }
                else
                {
