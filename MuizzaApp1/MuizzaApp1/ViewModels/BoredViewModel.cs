...
{
    public partial class BoredViewModel : ObservableObject
    {
        private readonly IListChoiceService _listChoiceService;

        public BoredViewModel(IListChoiceService listChoiceService)
        {
        ...
        private async Task NavigateToListChoice(string choice)
        {
            try
            {
                ...
                if (listChoice != null)
                {
                    ...
                    await Shell.Current.GoToAsync($"{nameof(ListChoice)}?emotion=Bored&choice={Uri.EscapeDataString(listChoice.Title)}");
                }
                else
                {
